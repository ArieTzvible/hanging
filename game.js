...
//   "בטטה",
// ];
// const dairyProducts = ["חלב", "גבינה", "קוטג", "לבן", "יוגורט"];
const myLetters = [
  "א",
  "ב",
  "ג",
...
// const myImages =['img1/2-01.png', 'img1/3-01.png', 'img1/4-01.png', 'img1/5-01.png','img1/6-01.png', 'img1/7-01.png', 'img1/8-01.png', 'img1/9-01.png', 'img1/10-01.png',]
...
function gameSelection(value) {
  ...
  document.querySelector(
    "#id_img"
  ).innerHTML = `<img src="img1/2523 [Converted]-01.png">`;
  document.querySelector(
    "#div_new_game"
  ).innerHTML = `<button onClick="window.location.reload();">משחק חדש</button>`;
document.body.innerHTML+=`<div id="div_first_boxs_letters">
<div id="div_boxs_letters"></div>
</div>`
  for (let i = 0; i < myLetters.length; i++) {
    console.log("v");
    document.querySelector(
      "#all_letters"
    ).innerHTML += `<div id="div_lett${i}"><button class="hebrew_letters" id="lett${i}" value="${myLetters[i]}" onclick="clickLetter(this)">${myLetters[i]}</button></div>`;
  }
    ...
    if (clickPage === randomWordChoice[i]) {
      ...
      for (let i = 0; i < randomWordChoice.length; i++) {
        ...
        if (j === randomWordChoice.length-1) {
          document.querySelector(
            "#id_img"
          ).innerHTML = `<img src="img1/10-01.png">`;
          removeButton = document.getElementById("div_boxs_letters");
          removeButton.remove();
          document.getElementById(
            "all_letters"
          ).innerHTML = `<h2>כל הכבוד הצלת את זלנסקי</h2>`;
          user["wins"]++
